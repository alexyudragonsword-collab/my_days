...
- **统计报表**：周报 / 月报视图——完成率、每日完成柱状图、游玩与咨询时长、训练容量、发布内容、日均热量与体重变化
- **自媒体**：灵感 → 待策划 → 制作中 → 待发布 → 已发布 五阶段看板，可按平台筛选，发布链接与播放/点赞/评论数据
- **开发工作**：项目、里程碑、工作项（功能 / 需求 / Bug / 技术问题）、开发日志
- **咨询工作**：客户、咨询项目、沟通记录（含时长与费用）、交付物、跟进提醒、按月收入统计（费用与结算状态）
- **健身计划**：训练模板（可一键复制再改）、逐组记录次数重量、上次同动作数据、身体数据与趋势
- **饮食计划**：每日营养目标、计划餐食与实际摄入、常用食物、餐食模板（常吃的一餐存下来一键套用）、当日热量蛋白质汇总
- **游戏娱乐**：清单状态、游玩计时、进度与下一次目标、累计时长（没有逾期压力）
- **数据与设置**：数据文件信息、手动 / 每日自动备份、安全恢复、ZIP 导出（JSON + CSV）、回收站、首页摘要卡片的显隐与顺序、主题等偏好
...
| 统计报表（周报 / 月报） |
| --- |
| ![统计报表](docs/screenshots/report.png) |
...
- `Ctrl / Command + K` 打开全局搜索，可跨模块搜索并直接跳转到记录；搜索走 SQLite 全文索引，中文关键词支持任意位置的子串匹配。
- 更多快捷键：`N` 快速新增、`1`~`9` / `0` 按侧栏顺序切换页面、`?` 显示快捷键列表（在输入框里打字时不会触发）。
...
- **系统托盘**：托盘菜单可打开主窗口、打开数据目录、检查更新、保存并退出；两个可选项默认关闭——「关闭窗口时保留在托盘」和「开机自动启动」。
- **自动更新**：Windows / Linux 会在后台下载新版本并询问是否重启安装；macOS 因安装包未公证无法自动替换，只提示前往发布页手动下载。
