...
function CommForm(props: { projectId: number; onDone: () => void }) {
  ...
  return (
    <Modal title="记录沟通 / 会议" onClose={props.onDone}>
      <div className="grid-2">
        ...
        <Field label="形式">
          <select className="input" value={form.form} onChange={(e) => set('form', e.target.value)}>
            {['线上会议', '电话', '当面', '微信/消息', '邮件'].map((s) => <option key={s}>{s}</option>)}
          </select>
        </Field>
        <Field label="咨询时长（分钟）">
      ...
      <div className="row" style={{ justifyContent: 'flex-end' }}>
        <button className="btn" onClick={props.onDone}>取消</button>
  ...
  return (
    <div>
      <div className="page-head">
        <h2>咨询工作</h2>
        <input className="input" style={{ width: 180 }} placeholder="添加客户，回车保存" value={newClient}
          onChange={(e) => setNewClient(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addClient()} />
      </div>
      <QueryView
        query={clientsQuery}
        isEmpty={() => clients.length === 0}
        empty={<EmptyState icon="💼" text="还没有客户" hint="先添加一个客户，再为客户建立咨询项目" />}
      >
        {() => (
              ...
              <div>
                <div className="card">
                  <div className="row" style={{ justifyContent: 'space-between' }}>
                    ...
                    <div className="row">
                      <button className="btn small" onClick={async () => {
                        const note = window.prompt('客户备注', String(client.note || ''));
                        if (note !== null) {
                      ...
                      <Dropdown>
                        <button onClick={async () => {
                          if (await confirm({ title: '归档客户', body: `归档「${client.name}」及其显示？` })) {
                            await updateRow('consult_clients', client.id, { archived: 1 });
                            invalidateTable('consult_clients');
                          }
                        }}>归档客户</button>
                        <button className="danger" onClick={() => softDelete('consult_clients', client.id, '客户')}>删除客户</button>
                      </Dropdown>
                    </div>
                  </div>
                  {client.note ? <p className="small muted" style={{ marginBottom: 0 }}>{String(client.note)}</p> : null}
                  <div className="row" style={{ marginTop: 10 }}>
                    <input className="input" style={{ flex: 1 }} placeholder="为该客户添加咨询项目，回车保存" value={newProject}
                  ...
                  <div className="row wrap" style={{ marginTop: 8 }}>
                    ...
                    {projects.length === 0 && <span className="small muted">该客户暂无咨询项目</span>}
                  </div>
                </div>
                ...
                {project && (
                  <>
                    <div className="card">
                      <div className="row" style={{ justifyContent: 'space-between' }}>
                        <h3 style={{ margin: 0 }}>{String(project.name)} <span className="sub">{String(project.status)}</span></h3>
                        <div className="row">
                          <button className="btn small" onClick={async () => {
                            const req = window.prompt('当前需求', String(project.requirement || ''));
                            if (req !== null) {
                          ...
                          <Dropdown>
                            <button onClick={() => addToPlan({ title: `咨询：${project.name}`, sourceModule: 'consult_project', sourceId: project.id })}>加入今日计划</button>
                            <button onClick={async () => {
                              if (await confirm({ title: '归档项目', body: `归档「${project.name}」？` })) {
                                await updateRow('consult_projects', project.id, { archived: 1 });
                                invalidateTable('consult_projects');
                              }
                            }}>归档项目</button>
                            <button className="danger" onClick={() => softDelete('consult_projects', project.id, '咨询项目')}>删除项目</button>
                          </Dropdown>
                        </div>
                      </div>
                      {project.requirement ? <p className="small muted">{String(project.requirement)}</p> : null}
                      <div className="stat-row">
                        <div className="stat"><div className="num">{fmtMinutes(totalMinutes) || '0'}</div><div className="lbl">累计咨询时长</div></div>
                        <div className="stat"><div className="num">￥{totalFee.toFixed(0)}</div><div className="lbl">累计费用</div></div>
                        <div className="stat"><div className="num">￥{unsettled.toFixed(0)}</div><div className="lbl">未结算</div></div>
                      </div>
                    </div>

                    <div className="card">
                      <div className="row" style={{ justifyContent: 'space-between', marginBottom: 8 }}>
                        <h3 style={{ margin: 0 }}>沟通 / 会议</h3>
                        <button className="btn primary small" onClick={() => setCommOpen(true)}>＋ 记录沟通</button>
                      </div>
                      {comms.length === 0
                        ? <p className="small muted" style={{ margin: 0 }}>暂无沟通记录</p>
                        : comms.slice(0, 10).map((c) => (
                          <div className="list-item small" key={c.id}>
                            <span className="badge">{fmtDateTime(String(c.time))}</span>
                            <span className="badge accent">{String(c.form || '')}</span>
                            <span className="title">{String(c.notes || '')}</span>
                            {c.duration_min ? <span className="muted">{fmtMinutes(Number(c.duration_min))}</span> : null}
                            {c.fee_amount != null ? (
                              <span className={'badge ' + (c.settled ? 'ok' : 'warn')}>￥{String(c.fee_amount)}{c.settled ? ' 已结' : ' 未结'}</span>
                            ) : null}
                            <Dropdown>
                              ...
                              <button onClick={() => addToPlan({ title: `会议：${project.name}`, sourceModule: 'consult_comm', sourceId: c.id })}>加入今日计划</button>
                              <button className="danger" onClick={() => softDelete('consult_comms', c.id, '沟通记录')}>删除</button>
                            </Dropdown>
                          </div>
                        ))}
                    </div>
                    ...
                    <div className="card">
                      ...
                      <div className="row" style={{ marginBottom: 8 }}>
                        <input className="input" style={{ flex: 1 }} placeholder="交付物名称" value={newDeliverable.name}
                      ...
                      {deliverables.length === 0
                        ? <p className="small muted" style={{ margin: 0 }}>暂无交付物</p>
                        : deliverables.map((d) => (
                          <div className="list-item" key={d.id}>
                            ...
                            {d.due_date ? (
                              <span className={'badge ' + (String(d.due_date) < todayStr() && d.status !== '已完成' ? 'danger' : '')}>
                                截止 {String(d.due_date)}
                              </span>
                            ) : null}
                            <Dropdown>
                              <button onClick={async () => {
                                const nd = window.prompt('延期到（YYYY-MM-DD）', String(d.due_date || addDays(todayStr(), 7)));
                              ...
                              <button onClick={() => addToPlan({ title: `交付：${d.name}`, sourceModule: 'consult_deliverable', sourceId: d.id })}>加入今日计划</button>
                              <button className="danger" onClick={() => softDelete('consult_deliverables', d.id, '交付物')}>删除</button>
                            </Dropdown>
                          </div>
                        ))}
                    </div>
                    ...
                    <div className="card">
                      ...
                      <div className="row" style={{ marginBottom: 8 }}>
                        ...
                        <input className="input" style={{ flex: 1 }} placeholder="跟进内容" value={newFollowup.content}
                      ...
                      {followups.length === 0
                        ? <p className="small muted" style={{ margin: 0 }}>暂无跟进安排</p>
                        : followups.map((f) => (
                          <div className="list-item" key={f.id}>
                            ...
                            <Dropdown>
                              <button onClick={() => addToPlan({ title: `跟进：${f.content}`, sourceModule: 'consult_followup', sourceId: f.id })}>加入今日计划</button>
                              <button className="danger" onClick={() => softDelete('consult_followups', f.id, '跟进')}>删除</button>
                            </Dropdown>
                          </div>
                        ))}
                    </div>
